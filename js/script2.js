...
$(function() {
    ...
    const eventDay = Date.parse("2023/08/04 23:59:59")
    const eventHour = 23
    const eventMin = 59
    const eventSec = 59
    const today = new Date();
    ...
    let time = setInterval(function() {
        ...
        let resultSec = `${eventSec - sec}` //해당영역에서 오류발생

        //2번째 적용
        /* const result = eventDay - today;
        let resultHr = Math.floor(result/ (1000*60*60))
        let resultMin = Math.floor(result/ (1000*60))
        let resultSec = Math.floor(result/ (1000))

        const h = resultHr
        const m = resultMin - resultHr*60;
        const s = resultSec - resultMin*60; */
